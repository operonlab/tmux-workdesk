...
# The demo presses prefix+g / +e / +a / +i in turn; we show the "⌃B" cap then
...
for b in assets/bezel-cb.png assets/bezel-cbg.png assets/bezel-cbe.png assets/bezel-cba.png assets/bezel-cbi.png; do
  [ -f "$b" ] || { echo "$b missing — run keycast-bezel.py first" >&2; exit 1; }
done

ffmpeg -y -loglevel error -i demo-raw.gif \
  -i assets/bezel-cb.png -i assets/bezel-cbg.png -i assets/bezel-cbe.png \
  -i assets/bezel-cba.png -i assets/bezel-cbi.png \
  -filter_complex "\
[0:v][1:v]overlay=(W-w)/2:H-h-120:enable='between(t,1.70,2.15)+between(t,4.70,5.15)+between(t,7.70,8.20)+between(t,10.75,11.25)'[v1];\
[v1][2:v]overlay=(W-w)/2:H-h-120:enable='between(t,2.15,3.30)'[v2];\
[v2][3:v]overlay=(W-w)/2:H-h-120:enable='between(t,5.15,6.30)'[v3];\
[v3][4:v]overlay=(W-w)/2:H-h-120:enable='between(t,8.20,9.35)'[v4];\
[v4][5:v]overlay=(W-w)/2:H-h-120:enable='between(t,11.25,12.40)'[v5];\
